...
public class TradeBuffer {
    ...
    public List<Trade> getTradeList(String agentId){
        ...
        synchronized (buffer) {
        	for( int i = 0; i < buffer.size(); i++ ){
            	if( null != buffer.get(i) && null != buffer.get(i).getTrade() ){
                    ...
                    if( agentId.equals ( trawg.getTrade().getBuyId() ) && trawg.getBuyAgentTag() != 0 ){
                    	trawg.minusBuyAgentTag();
                        traList.add( buffer.get(i).getTrade() );
                    }
                    else if( agentId.equals ( trawg.getTrade().getSellId() ) && trawg.getSellAgentTag() != 0 ){
